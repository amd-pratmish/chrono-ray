...
FLAG_CIRCULAR_PLATE = True 

PLATE_Z = 0.01
PLATE_DENSITY = 7.8e3 #steel plate 

if FLAG_CIRCULAR_PLATE:
    PLATE_RADIUS = 0.015
    PLATE_MASS = PLATE_DENSITY * math.pi * PLATE_RADIUS**2 * PLATE_Z
...
def _make_run_dir(config):
    ...
    trial_dir = os.path.join(os.getcwd(), name)
...
def simulate_fn(config):
    ...
    if FLAG_CIRCULAR_PLATE:
        I = PLATE_MASS * (PLATE_RADIUS**2) / 2
        pp_vis_shape = chrono.ChVisualShapeCylinder(PLATE_RADIUS, PLATE_Z)
    else:
    ...
    if FLAG_CIRCULAR_PLATE:
        chrono.AddCylinderGeometry(pp, ss_c_material, 
                          PLATE_RADIUS, PLATE_Z,
                          chrono.ChVector3d(0, 0, 0),
    ...
    if FLAG_CIRCULAR_PLATE:
        pp_bce = sys_sph.CreatePointsCylinderInterior(PLATE_RADIUS, PLATE_Z, True)
    else:
    ...
    if FLAG_VISUALIZE:
        ...
        vis.AddCamera(
            chrono.ChVector3d(0.40, -0.40, 0.25),   # camera position
            chrono.ChVector3d(0,    0,     0.09),   # look-at point
        )
    ...
    while sim_time < t_end:
        ...
        # Log every `log_steps` simulation steps
        if step % log_steps == 0:
            pos = pp.GetPos()
