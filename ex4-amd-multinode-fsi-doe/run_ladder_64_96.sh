#!/usr/bin/env bash
# Scale ladder: 64 → 72 → 96 → 128 (skip tiers already PASS).
# Partitions: tries lux then rad-burst per tier via submit_scale.sh.
set -uo pipefail

EX4="/home/pratmish/chrono-ray/ex4-amd-multinode-fsi-doe"
LADDER=(64 72 96 128)
MAX_ATTEMPTS=5
LOG="${EX4}/logs/ladder_64_96_$(date +%Y%m%d_%H%M%S).log"
exec > >(tee -a "${LOG}") 2>&1
...
echo "=== Ladder 64 → 72 → 96 → 128 started $(date) ==="
echo "Partition order per tier: lux → rad-burst (via submit_scale.sh)"
echo "Log: ${LOG}"
pkill -f run_ladder_24_32_plus.sh 2>/dev/null || true
