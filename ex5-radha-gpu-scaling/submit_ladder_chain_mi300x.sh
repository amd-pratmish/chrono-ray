#!/usr/bin/env bash
# Submit MI300X ladder as chained Slurm jobs (mi300x max walltime 2h per job).
# Usage: bash submit_ladder_chain_mi300x.sh [after_job_id]
set -euo pipefail
...
for SCALE in 24 32 64; do
    submit_tier "${SCALE}" || echo "extended tier ${SCALE} unavailable on Radha mi300x"
done

echo "MI300X chain last job: ${PREV:-none}"
