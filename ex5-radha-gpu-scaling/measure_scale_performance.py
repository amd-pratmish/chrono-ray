...
def collect_runs(platform: str) -> list[dict]:
    rows: list[dict] = []
    results = RESULTS_ROOT / platform
    ...
    for run_dir in sorted(results.glob("scale*gpu_*")):
        ...
        cluster_gpus = float(
            summary.get("ray_gpus_registered")
            or summary.get("cluster_gpus")
            or scale_gpus
            or 0
        )
        max_conc = int(summary.get("max_concurrent", 0) or 0)
        wall_s = float(summary.get("wall_elapsed_s") or summary.get("elapsed_seconds") or 0)
        ...
        rows.append(
            {
                ...
                "tier": summary.get("scale_tier", scale_gpus if scale_gpus is not None else ""),
                "mpi_ranks_equiv": summary.get("max_concurrent", max_conc),
                "job_id": summary.get("slurm_job_id") or summary.get("job_id", run_dir.name.split("_")[-1]),
                "status": status,
                ...
                "max_concurrent": max_conc or summary.get("max_concurrent", ""),
                ...
                "aggregate_gpu_sph_steps_per_s": summary.get("aggregate_gpu_sph_steps_per_s", ""),
                "mean_gpu_sph_steps_per_s": summary.get("mean_gpu_sph_steps_per_s", ""),
                "total_gpu_sph_steps": summary.get("total_gpu_sph_steps", ""),
                "ray_scheduling_overhead_s": summary.get("ray_scheduling_overhead_s", ""),
                "gpu_sph_fraction_of_doe": summary.get("gpu_sph_fraction_of_doe", ""),
                "gpu_link_type": summary.get("gpu_link_type", ""),
                "xgmi_link_count": summary.get("xgmi_link_count", ""),
                "gpu_link_speed_gts": summary.get("gpu_link_speed_gts", ""),
                "gpu_mem_util_pct": summary.get("gpu_mem_util_pct", ""),
                "gpu_bw_util_pct": summary.get("gpu_bw_util_pct", ""),
                "mpi_unique_nodes": summary.get("mpi_unique_nodes", ""),
                "mpi_cross_node_traffic": summary.get("mpi_cross_node_traffic", ""),
                "ray_unique_nodes": summary.get("ray_unique_nodes", ""),
                "ray_cross_node_traffic": summary.get("ray_cross_node_traffic", ""),
                "gpu_topo_summary": (summary.get("gpu_topo_summary", "") or "")[:120],
...
FIELDNAMES = [
    ...
    "aggregate_gpu_sph_steps_per_s",
    "mean_gpu_sph_steps_per_s",
    "total_gpu_sph_steps",
    "ray_scheduling_overhead_s",
    "gpu_sph_fraction_of_doe",
    "gpu_link_type",
    "xgmi_link_count",
    "gpu_link_speed_gts",
    "gpu_mem_util_pct",
    "gpu_bw_util_pct",
    "mpi_unique_nodes",
    "mpi_cross_node_traffic",
    "ray_unique_nodes",
    "ray_cross_node_traffic",
    "gpu_topo_summary",
    "partition",
    "nodelist",
    "avg_sec_per_trial",
...
def write_markdown(rows: list[dict], path: Path) -> None:
    ...
    lines.extend([
        "",
        "## Interconnect / network (PASS runs)",
        "",
        "| Tier | Job | GPU link | xGMI links | Nodes | Cross-node | GPU SPH steps/s |",
        "|------|-----|----------|------------|-------|------------|-----------------|",
    ])
    for r in rows:
        if r["status"] != "PASS":
            continue
        lines.append(
            f"| {r.get('tier', r['scale_gpus'])} | {r['job_id']} | {r.get('gpu_link_type', '?')} | "
            f"{r.get('xgmi_link_count', '?')} | {r.get('mpi_unique_nodes', r.get('ray_unique_nodes', '?'))} | "
            f"{r.get('mpi_cross_node_traffic', '?')} | {r.get('aggregate_gpu_sph_steps_per_s', '?')} |"
        )
...
def main() -> int:
    ...
    ap.add_argument("--platform", default=os.environ.get("CHR_RAY_PLATFORM", "mi350x"))
    args = ap.parse_args()
    rows = collect_runs(args.platform)
    report_dir = RESULTS_ROOT / args.platform / "performance"
