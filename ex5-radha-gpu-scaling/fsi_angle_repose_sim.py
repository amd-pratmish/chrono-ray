...
def simulate_fn(config):
    ...
    steps_per_s = gpu_sph_steps / gpu_wall if gpu_wall > 0 else 0.0

    if save_particles and (save_end_only or out_frame == 0):
        sys_sph.SaveParticleData(sph_dir)

    metrics = {
        "gpu_sph_steps": gpu_sph_steps,
        "gpu_sph_wall_seconds": round(gpu_wall, 4),
        "gpu_sph_steps_per_s": round(steps_per_s, 2),
        "sim_time_end_s": round(sim_time, 6),
        "hostname": os.environ.get("HOSTNAME", ""),
    }
    with open(os.path.join(run_dir, "trial_metrics.json"), "w") as mf:
        json.dump(metrics, mf, indent=2)

    print(
        f"[trial done] steps={gpu_sph_steps} wall={gpu_wall:.2f}s "
        f"steps/s={steps_per_s:.1f} {run_dir}",
        flush=True,
    )
